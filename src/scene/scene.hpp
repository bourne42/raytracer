...
public:
    ...
    */
	...
	Matrix4* transMat;
	Matrix4* inverseTransMat;
	Matrix3* normalMat;

	Vector4 getV4(Vector3 in, int end);

	float triangleIntersection(Vector3 eye, Vector3 del, float startInterval, float endInterval,
	...
	*/
	...
	/**
	 * returns the t for the intersection of the ray given by e+td between start and end interval
	If there is no intersection return 0
	Interval will always return >0 because the ray won't go backwards (I'm pretty sure it shouldn't be 0)
	Declared to be 0 because abstract, must be implmeneted in sphere, triangle, and mesh
	*/
	virtual float getIntersection(Vector3 e, Vector3 d, float startInterval, float endInterval, PixelInfo* intersection) =0;

	Color3 getTextureColor(Vector2 texCoord, const Material *mat);
